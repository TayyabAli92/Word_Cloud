...
def get_image_download_link(buffered, format_):
    ...
    return f'<a href="data:image/{format_};base64,{image_base64}" download="WordCloud.{format_}">Download {format_}</a>'

# function to generate a download link a DataFrame
def get_table_download_link(df, filename, file_label):
    csv = df.to_csv(index=False)
    b64 = base64.b16encode(csv.encode()).decode()
    return f'<a herf="data:file/csv;base64, {b64}" download="{filename}">{file_label}</a>' 
...
if uploaded_file:
    ...
    # check the file and read the file    
    ...
    st.sidebar.markdown("Created by: [Muhammad Tayyab Ali](https://github.com/TayyabAli92)")
    st.sidebar.markdown("Contact: [Email](hameshak110@gmail.com)")
